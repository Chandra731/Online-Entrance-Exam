...
class ExamPage {
  ...
  async init() {
    try {
      ...
      // Check if user already submitted exam
      const userId = AuthService.currentUser.uid;
      const results = await ExamService.getAllResults();
      const hasSubmitted = results.success && results.results.some(result => result.studentId === userId);
      if (hasSubmitted) {
        showAlert('You already attempted the exam please wait for the counselor message.', 'warning');
        // Disable start exam button and other controls
  ...
  async loadExamData() {
    try {
      ...
      // Removed setting examCode textContent as user does not want to display exam code
  ...
  showGuidelinesModal() {
    ...
    // modal._element.setAttribute('inert', ''); // Removed inert attribute to restore scrollability
    modal.show();
  }
