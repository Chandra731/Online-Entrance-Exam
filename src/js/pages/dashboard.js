...
class DashboardPage {
  ...
  async loadDashboardData() {
    try {
      // Load students, exam results, and participant snapshots in parallel
      const [studentsResult, resultsResult, snapshotsResult] = await Promise.all([
        CounselorService.getStudents(),
        ExamService.getAllResults(),
        CounselorService.getParticipantSnapshots()
      ]);
      ...
      if (snapshotsResult.success) {
        ...
        snapshotsResult.snapshots.forEach(snapshot => {
          this.participantSnapshots.set(snapshot.userId, snapshot.latestSnapshotBase64);
        });
      } else {
        console.error('Error loading participant snapshots:', snapshotsResult.error);
      }

      // Update UI
      this.updateStats();
      this.renderResults();
      this.renderWebcamMonitoringSection();

      // Start snapshot refresh interval
  ...
  renderWebcamMonitoringSection() {
    const container = document.getElementById('webcamMonitoringSection');
    if (!container) return;

    container.innerHTML = '';

    if (this.participantSnapshots.size === 0) {
      container.innerHTML = `
        <div class="text-center text-muted py-4">
          <i class="bi bi-camera-video-off fs-1 mb-3"></i>
          <p>No webcam snapshots available</p>
        </div>
      `;
      return;
    }
    ...
    this.participantSnapshots.forEach((base64Image, userId) => {
      const student = this.students.find(s => s.id === userId);
      const studentName = student ? student.name : 'Unknown';

      snapshotElements.push(`
        <div class="webcam-snapshot-card text-center m-2" style="width: 120px;">
          <img src="${base64Image}" alt="Webcam snapshot" class="rounded mb-2" style="width: 100px; height: 100px; object-fit: cover; border: 1px solid #ccc;">
          <div class="text-truncate" title="${studentName}">${studentName}</div>
        </div>
      `);
    });

    container.innerHTML = `
      <h5 class="mb-3">Student Webcam Monitoring</h5>
      <div class="d-flex flex-wrap">
        ${snapshotElements.join('')}
      </div>
    `;
  }
  ...
  startSnapshotRefresh() {
    ...
    this.snapshotIntervalId = setInterval(async () => {
      try {
        const snapshotsResult = await CounselorService.getParticipantSnapshots();
        if (snapshotsResult.success) {
          this.participantSnapshots.clear();
          snapshotsResult.snapshots.forEach(snapshot => {
            this.participantSnapshots.set(snapshot.userId, snapshot.latestSnapshotBase64);
        ...
        console.error('Error refreshing participant snapshots:', error);
      }
    }, 5000);
  }
